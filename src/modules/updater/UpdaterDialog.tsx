...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S neira-bin";
    case "debian":
      return `sudo apt install ./Neira_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Neira-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Neira v${manual.version} is available`
                  : `Neira v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Neira to finish installing."
              : downloading
                ? progress !== null
