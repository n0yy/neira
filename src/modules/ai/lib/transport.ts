...
async function readNeiraMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/NEIRA.md`;
  ...
  try {
    ...
    const content =
      r.content.length > NEIRA_MD_MAX_BYTES
        ? r.content.slice(0, NEIRA_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readNeiraMd(live.workspaceRoot);
