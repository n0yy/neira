...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("neira:ai-attach-file", onAttach);
    return () => window.removeEventListener("neira:ai-attach-file", onAttach);
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<neira-command name="${outcome.commandName}" />`;
        }
      }
    }
