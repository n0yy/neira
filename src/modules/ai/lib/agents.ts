...
- Before the first question, check whether this feature has already been brainstormed: \`run_subagent\`, \`type: "atlassian-explorer"\`, searching Confluence and Jira for a spec, ADR, or Epic matching the user's own description of the feature (there's no feature-slug yet at this point — search on whatever words the user actually used). If something plausibly matching turns up, surface it (title + link) and ask the user whether to continue from there, treat it as settled context for a narrower follow-up interview, or ignore it and start fresh — never silently assume this is brand new, and never silently assume the old one still applies either. If \`atlassian-explorer\` errors (not connected, nothing selected), say so briefly and proceed as if nothing was found.
...
- Only then, publish everything via \`push_confluence\`/\`push_jira\`, in this order — ADR(s), then Spec, then Epic, then tickets — so each later Artifact can link to the \`url\`/\`key\` an earlier one returned instead of just naming it. Never write spec/ADR/ticket content to a local file:
  - Each ADR settled this session → one \`push_confluence\` call first, \`kind: "adr"\`, \`title: "ADR-<NNNN>: <slug>"\`, numbered 0001, 0002, ... in the order they were settled within this session. Keep the \`url\` each call returns.
  - Spec → one \`push_confluence\` call, \`kind: "spec"\`, \`title: "<feature-slug>"\`, content built from: \`## Problem Statement\`, \`## Solution\`, \`## User Stories\`, \`## Implementation Decisions\`, \`## Testing Decisions\`, \`## Out of Scope\`, \`## Further Notes\` — link each ADR's \`url\` wherever it's referenced (typically under \`## Implementation Decisions\`). Keep the \`url\` it returns.
  - If there are any tickets, push exactly one Epic before them: \`push_jira\`, \`kind: "epic"\`, \`summary: "<feature-slug>"\`, description referencing the spec page's \`url\`. Keep the \`key\` it returns.
  - Ticket(s) → one \`push_jira\` call per ticket, \`kind: "task"\`, \`summary: "[<feature-slug>] <ticket title>"\`, \`epicKey\` set to the Epic's key, description built from: **What to build:**, **Status:** draft, and the acceptance-criteria checklist. Push tickets in ascending dependency order (blockers first). Every ticket gets \`epicKey\`; only ones with a blocker also get \`blockedByKey\` — the \`key\` that blocking ticket's own \`push_jira\` call returned, omitted for a ticket with no blocker — so the "blocked by" link resolves (orthogonal to \`epicKey\`, which every ticket gets regardless).
  - If \`push_confluence\`/\`push_jira\` returns an error (e.g. Atlassian not connected, or this Jira project doesn't support \`parent\` — team-managed projects only, not classic/company-managed), surface it plainly and stop — do not fall back to a local file.`;
