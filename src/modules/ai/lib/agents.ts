...
- Only then, publish everything via \`push_confluence\`/\`push_jira\` — never write spec/ADR/ticket content to a local file:
  - Spec → one \`push_confluence\` call, \`kind: "spec"\`, \`title: "<feature-slug>"\`, content built from: \`## Problem Statement\`, \`## Solution\`, \`## User Stories\`, \`## Implementation Decisions\`, \`## Testing Decisions\`, \`## Out of Scope\`, \`## Further Notes\`. Keep the \`url\` it returns.
