...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-neira-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-neira-slot", String(slots.length));
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[neira] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[neira] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[neira] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[neira-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[neira-webgl] dispose failed:", e);
  }
  try {
    const r = (
