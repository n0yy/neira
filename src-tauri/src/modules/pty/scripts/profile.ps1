# neira-shell-integration (PowerShell)
...
if ($global:__NEIRA_HOOKS_LOADED) { return }
$global:__NEIRA_HOOKS_LOADED = $true

if ($env:NEIRA_CLI -and (Test-Path -LiteralPath $env:NEIRA_CLI -PathType Leaf)) {
    function global:neira {
        & $env:NEIRA_CLI @args
    }
}
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__neira_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__neira_urlencode {
    param([string]$s)
...
function global:__neira_install_readline {
    if ($global:__neira_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__neira_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__neira_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __neira_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__neira_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__neira_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:NEIRA_BLOCKS) {
        ...
        $gap = if ($global:__neira_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__neira_user_prompt) {
        try { & __neira_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
