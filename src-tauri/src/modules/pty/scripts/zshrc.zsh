...
# zsh, so we shadow $? into `_neira_ret`.

{
  _neira_user_zdotdir="${NEIRA_USER_ZDOTDIR:-$HOME}"
  [ -f "$_neira_user_zdotdir/.zshrc" ] && source "$_neira_user_zdotdir/.zshrc"
...
if [[ -z "$__NEIRA_HOOKS_LOADED" ]]; then
  ...
  _neira_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_neira_urlencode "$PWD")"
  ...
  _neira_preexec() {
    ...
    [[ -n "$NEIRA_BLOCKS" ]] && _neira_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _neira_precmd
    add-zsh-hook preexec _neira_preexec
  fi
