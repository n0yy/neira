<div align="center">
  ...
  <p>
    ...
    ·
    <a href="https://neira.app/docs">Docs</a>
    ·
    <a href="https://github.com/crynta/Neira-website">Website's source code</a>
  </p>
...
- **Agentic workflow:** plans, sub-agents, project memory via `NEIRA.md`, file read / write / edit / multi-edit / grep / glob, bash with approval gating, background processes
...
Latest installers are on the [Releases](https://github.com/crynta/neira/releases/latest) page. Neira auto-updates from there.
...
- **Arch / AUR:** `yay -S neira-bin` (or `paru`, etc.). Tracks the latest release.
- **NixOS / Nix**: use the official flake - `nix profile install github:crynta/neira` (non-NixOS), or import the flake and add `inputs.neira.packages.${pkgs.system}.neira` to `environment.systemPackages` (NixOS). The `nixosModules.neira` output is also available for a simpler setup.
- **AppImage:** needs FUSE. Without it: `./Neira_*.AppImage --appimage-extract-and-run`. On Wayland with rendering glitches, try `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Otherwise the `.deb` / `.rpm` packages link against the system GTK stack and tend to be smoother.

## Configure AI

1. Open **Settings -> AI**.
2. Pick a provider and paste your API key. For local inference, point Neira at your LM Studio / MLX / Ollama endpoint.
...
Neira is licensed under the Apache-2.0 License. For more information on our dependencies, see [Apache License 2.0](LICENSE).
