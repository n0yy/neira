<div align="center">
  ...
  <p><a href="https://neira.app">Сайт</a> · <a href="https://neira.app/docs">Документация</a> · <a href="https://github.com/crynta/Neira-website">Исходный код сайта</a></p>

  <p>
    <img src="https://img.shields.io/github/v/release/crynta/neira?label=version&color=blue" alt="версия" />
    <img src="https://img.shields.io/github/downloads/crynta/neira/total?label=downloads&color=blue" alt="загрузки" />
...
- **Агентный процесс:** планы, субагенты, память проекта через `NEIRA.md`, чтение / запись / правка / множественная правка / grep / glob, bash с подтверждением и фоновые процессы
...
Последние установщики находятся на странице [Releases](https://github.com/crynta/neira/releases/latest). Neira автоматически обновляется оттуда.
...
- **Arch / AUR:** `yay -S neira-bin` или `paru`. Пакет следует за последним выпуском.
- **NixOS / Nix:** используйте официальный flake. Вне NixOS выполните `nix profile install github:crynta/neira`. В NixOS импортируйте flake и добавьте `inputs.neira.packages.${pkgs.system}.neira` в `environment.systemPackages`. Для более простой настройки доступен `nixosModules.neira`.
- **AppImage:** требует FUSE. Без него выполните `./Neira_*.AppImage --appimage-extract-and-run`. При проблемах рендеринга в Wayland попробуйте `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Пакеты `.deb` / `.rpm` используют системный стек GTK и обычно работают плавнее.
...
Neira распространяется по лицензии Apache-2.0. Информацию о зависимостях см. в [Apache License 2.0](../../LICENSE).
