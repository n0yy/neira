<div align="center">
  ...
  <p>
    <a href="https://neira.app">Website</a> ·
    <a href="https://neira.app/docs">Dokumentation</a> ·
    <a href="https://github.com/crynta/Neira-website">Quellcode der Website</a>
  </p>

  <p>
    <img src="https://img.shields.io/github/v/release/crynta/neira?label=version&color=blue" alt="Version" />
...
- **Agentischer Workflow:** Pläne, Sub-Agenten, Projektgedächtnis über `NEIRA.md`, Lesen / Schreiben / Bearbeiten / Mehrfachbearbeitung / grep / glob, Bash mit Freigabe und Hintergrundprozesse
...
Die neuesten Installationspakete stehen auf der Seite [Releases](https://github.com/crynta/neira/releases/latest). Neira aktualisiert sich von dort automatisch.
...
- **Arch / AUR:** `yay -S neira-bin` (oder `paru` usw.). Folgt der neuesten Version.
- **NixOS / Nix:** Nutze den offiziellen Flake: `nix profile install github:crynta/neira` außerhalb von NixOS. Unter NixOS importierst du den Flake und fügst `inputs.neira.packages.${pkgs.system}.neira` zu `environment.systemPackages` hinzu. Für eine einfachere Einrichtung ist auch `nixosModules.neira` verfügbar.
- **AppImage:** Benötigt FUSE. Ohne FUSE: `./Neira_*.AppImage --appimage-extract-and-run`. Bei Darstellungsfehlern unter Wayland hilft möglicherweise `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Die `.deb`- / `.rpm`-Pakete binden stattdessen den GTK-Stack des Systems ein und laufen meist flüssiger.

## KI konfigurieren

1. Öffne **Einstellungen -> KI**.
2. Wähle einen Anbieter und füge deinen API-Schlüssel ein. Für lokale Inferenz verweist du Neira auf deinen LM Studio- / MLX- / Ollama-Endpunkt.
...
Neira steht unter der Apache-2.0-Lizenz. Weitere Informationen zu unseren Abhängigkeiten findest du in der [Apache License 2.0](../../LICENSE).
