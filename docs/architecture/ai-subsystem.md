...
2. Builds a stable system prompt from `selectSystemPrompt(modelId)` plus optional persona, custom instructions, and `NEIRA.md` project memory.
...
Conversations are organized into sessions. Persistence lives in `neira-sessions.json` via `tauri-plugin-store` (`src/modules/ai/lib/sessions.ts`):
